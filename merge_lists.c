...
int	main(void)
{
	...
	list2 = NULL;
	final = NULL;
	printf("Insert two sorted arrays of integers.\n");
	s = calloc(10000, 1);
	printf("Insert here the first array (in ascending order): ");
	fgets(s, 10000, stdin);
	ft_iterate_str(&list1, s);
	free(s);
	s = NULL;
	s = calloc(10000, 1);
	printf("Insert here the second array (in ascending order): ");
	fgets(s, 10000, stdin);
	ft_iterate_str(&list2, s);
	free(s);
	final = mergeTwoLists(list1, list2);
	printf("Merging two arrays... Printing...\n");
